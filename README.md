...
<p>
  ...
  for each different DBMS. The connection to each DBMS is added by selecting the server, user log-in information, and database route in the case of Firebase connections. 
  It doesn't necessarily have to be a localhost connection, it can be a connection on a local Wi-Fi network, although the loading time is slower than a localhost connection.
</p>
